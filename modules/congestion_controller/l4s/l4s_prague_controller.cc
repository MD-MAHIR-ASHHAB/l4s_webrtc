...
//                         << current_rtt.ms() << "ms, using default rate";
...
//     RTC_LOG(LS_WARNING)
//         << "Potential overflow in rate calculation, using default rate";
...
//     RTC_LOG(LS_INFO)
//         << "Prague: Applying congestion reduction due to CE marks. "
...
//     // // Rate limit updates to prevent excessive increases (minimum 200ms
//     // between rate increases) if (time_since_update < TimeDelta::Millis(50)) {
//     // Too frequent updates, return current rate without increase
//     //   return base_rate;
//     // Rate limit updates to prevent excessive increases (minimum 200ms between
//     // rate increases)
...
//     // Calculate increase as: (packet_size / current_cwnd) * (time_since_update
//     // / rtt)
...
//     // RTC_LOG(LS_INFO) << "Prague controller increase_factor=" <<
//     // increase_factor;

//     // Ensure increase factor is reasonable (prevent extreme values)
//     if (increase_factor < 0.99 ||
//         increase_factor > 1.05) {  // Very conservative: max 5% increase
//       RTC_LOG(LS_WARNING) << "Extreme increase factor: " << increase_factor
//                           << ", clamping (time_since_update="
//                           << time_since_update.ms() << "ms, rtt=" << rtt_seconds
//                           << "s)";
//       increase_factor = std::clamp(
//           increase_factor, 0.99, 1.05);  // Clamp to 1% decrease to 5% increase
//     }
...
//     // Log before another multiplication that might trigger unit_base.h
//     // assertion RTC_LOG(LS_INFO) << "Prague PRE-INCREASE-RATE-MULTIPLY:
//     // base_rate.bps()=" << base_rate.bps()
    ...
    if (rtt_seconds >= 0.005 && time_since_update.ms() >= 200) {
      ...
      if (increase_factor > 1.001) {
        RTC_LOG(LS_INFO) << "Prague additive increase: factor="
                         << increase_factor << ", rtt=" << rtt_seconds
